...
termux_msg(){
	[ "$1" == "info" ] && COLOR='\033[0;32m'  #green
	[ "$1" == "error" ] && COLOR='\033[0;31m' #red
	[ "$1" == "warning" ] && COLOR='\033[0;36m' #cyan
	NC='\033[0m' # No Color
	shift;
...
termux_download() {
	...
	termux_msg info "Downloading ${URL}"
	local TRYMAX=6
	for try in $(seq 1 $TRYMAX); do
		if curl -L --fail --retry 2 -o "$TMPFILE" "$URL"; then
			...
			termux_msg error "Download of $URL failed (attempt $try/$TRYMAX)" 1>&2
			sleep 45
		fi
	done
...
termux_step_start_build() {
	...
	if [ -n "${TERMUX_PKG_BLACKLISTED_ARCHES:=""}" ] && [ "$TERMUX_PKG_BLACKLISTED_ARCHES" != "${TERMUX_PKG_BLACKLISTED_ARCHES/$TERMUX_ARCH/}" ]; then
		termux_msg warning "Skipping building $TERMUX_PKG_NAME for arch $TERMUX_ARCH"
		exit 0
	fi
	...
	if [ -z "${TERMUX_SKIP_DEPCHECK:=""}" ]; then
		...
		for p in $TERMUX_ALL_DEPS; do
			termux_msg info "Building dependency $p if necessary..."
	...
	if [ "$TERMUX_DEBUG" == "true" ]; then
		if [ "$TERMUX_PKG_HAS_DEBUG" == "yes" ]; then
			...
			termux_msg info "Skipping building debug build for $TERMUX_PKG_NAME"
			exit 0
		fi
	else
		DEBUG=""
	...
	if [ -z "$TERMUX_DEBUG" ] &&
	   ...
	   [ -e "$TERMUX_TOPDIR/.built-packages/$TERMUX_PKG_NAME" ]; then
		if [ "$(cat "$TERMUX_TOPDIR/.built-packages/$TERMUX_PKG_NAME")" = "$TERMUX_PKG_FULLVERSION" ]; then
			termux_msg warning "$TERMUX_PKG_NAME@$TERMUX_PKG_FULLVERSION built - skipping (rm $TERMUX_TOPDIR/.built-packages/$TERMUX_PKG_NAME to force rebuild)"
			exit 0
		fi
	else
	...
	if [ -n "$TERMUX_PKG_BUILD_IN_SRC" ]; then
		termux_msg info "Building in src due to TERMUX_PKG_BUILD_IN_SRC being set" > "$TERMUX_PKG_BUILDDIR/BUILDING_IN_SRC.txt"
		TERMUX_PKG_BUILDDIR=$TERMUX_PKG_SRCDIR
	fi

	termux_msg info "termux - building $TERMUX_PKG_NAME for arch $TERMUX_ARCH..."
...
termux_step_finish_build() {
	termux_msg info "termux - build of '$TERMUX_PKG_NAME' done"
